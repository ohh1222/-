        ...
        "choices": [
            {
                ...
                "feedback": {
                    ...
                    "reg":   "坚持预设比例的解释未能令人信服。你要求公司在回复中提供每个资本化项目独立的技术可行性验证和商业化路径证明，并暂停审核进度。",
...
ENDINGS = {
    ...
    "invest": {
        ...
        "mid":  ("🎲", "信息博弈的参与者", "你意识到了风险，但行动有些迟缓。部分损失是因为信息不对称，部分是因为判断的犹豫。"),
...
.role-card.selected { border: 2px solid #1a73e8; background: #e8f0fe; }
.scene-box {
    background: #f8f9fa; border-radius: 12px;
    padding: 1.2rem 1.4rem; margin-bottom: 1rem;
}
.lens-box {
    border-left: 4px solid #1a73e8; background: #e8f0fe;
...
.verdict-box {
    background: #f8f9fa; border-radius: 12px;
    padding: 1rem 1.2rem; text-align: left; margin: 1rem 0;
}
...
def init():
    if "phase" not in st.session_state:
        st.session_state.phase = "intro"       # intro | scene | ending
        st.session_state.role = None
        st.session_state.scene_idx = 0
        st.session_state.scores = []
        st.session_state.chosen = None         # choice index for current scene
...
def get_ending_category(scores):
    total = sum(scores)
...
if st.session_state.phase == "intro":
    ...
    for i, r in enumerate(ROLES):
        with cols[i % 3]:
            ...
            if st.button(label, key=f"role_{r['id']}",
...
# ── SCENE SCREEN ──────────────────────────────────────────────────────────────
    ...
    if st.session_state.show_feedback and chosen_idx is not None:
        ...
        btn_text = "查看最终结局 →" if is_last else "下一个场景 →"
...
# ── ENDING SCREEN ─────────────────────────────────────────────────────────────
    ...
    cat = get_ending_category(scores)
    ...
    st.markdown(
        ...
        f"<p style='font-size:.8rem;font-weight:600;color:#888;margin-bottom:.5rem;text-transform:uppercase;letter-spacing:.05em'>案例对照分析</p>"
        f"<p>{VERDICT_TEXT[cat]}</p>"
        f"</div>",
        unsafe_allow_html=True
    )
