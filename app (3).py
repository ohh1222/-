...
[data-testid="stAppViewContainer"] { max-width: 740px; margin: auto; }

h2 { font-size: 1.6rem !important; font-weight: 700 !important; letter-spacing: -.01em; }
...
.scene-box {
    ...
    color: #c8d0e8 !important; font-size: .95rem; line-height: 1.75;
    box-shadow: 0 2px 12px rgba(0,0,0,.3);
}
.scene-box * { color: #c8d0e8 !important; }

.lens-box {
    background: linear-gradient(135deg, #0d1b3e 0%, #0a1628 100%);
    border: 1px solid #1e3a6e; border-left: 4px solid #4f8ef7;
    border-radius: 0 14px 14px 0; padding: 1rem 1.25rem; margin-bottom: 1.2rem;
    font-size: .92rem; line-height: 1.7; color: #a8c4f8 !important;
    box-shadow: 0 2px 12px rgba(15,50,120,.2);
}
.lens-box * { color: #a8c4f8 !important; }
.lens-box strong { color: #6fa3fb !important; font-weight: 600; }

.feedback-box {
    background: linear-gradient(135deg, #0d2b1e 0%, #091f16 100%);
    border: 1px solid #1a4d35; border-left: 4px solid #34c97a;
    border-radius: 0 14px 14px 0; padding: 1rem 1.25rem; margin-top: .9rem;
    font-size: .92rem; line-height: 1.7; color: #7ed4a8 !important;
    box-shadow: 0 2px 12px rgba(0,80,40,.2);
}
.feedback-box * { color: #7ed4a8 !important; }
.feedback-box strong { color: #34c97a !important; font-weight: 600; }

.verdict-box {
    background: linear-gradient(135deg, #1a1f2e 0%, #151922 100%);
    border: 1px solid #2e3650; border-radius: 14px;
    padding: 1.2rem 1.5rem; text-align: left; margin: 1rem 0;
    color: #c8d0e8 !important; line-height: 1.75;
    box-shadow: 0 2px 12px rgba(0,0,0,.3);
}
.verdict-box * { color: #c8d0e8 !important; }

[data-testid="stMetricValue"] {
    font-size: 1.6rem !important; font-weight: 700 !important; color: #e8edf8 !important;
}
[data-testid="stMetricLabel"] {
    font-size: .78rem !important; color: #7a8aaa !important; letter-spacing: .03em;
}
...
[data-testid="stButton"] > button {
    ...
    transition: all .18s ease !important; letter-spacing: .01em;
}
[data-testid="stButton"] > button[kind="secondary"] {
    background: #1a1f2e !important; border: 1px solid #2e3a55 !important; color: #c8d0e8 !important;
}
[data-testid="stButton"] > button[kind="secondary"]:hover {
    background: #222a40 !important; border-color: #4f6aaa !important;
    transform: translateY(-1px); box-shadow: 0 4px 14px rgba(0,0,0,.4) !important;
}
...
[data-testid="stButton"] > button[kind="primary"]:hover {
    ...
    transform: translateY(-1px); box-shadow: 0 4px 18px rgba(37,99,235,.5) !important;
}
hr { border-color: #2e3650 !important; margin: 1.5rem 0 !important; }
</style>
...
# ── ENDING ──
    ...
    # ending card colours per category
    card_cfg = {
        'good': ('#0d2b1e', '#1a4d35', '#34c97a', '#7ed4a8'),
        'mid':  ('#2b1f0a', '#4d3a10', '#f59e0b', '#fcd47a'),
        'bad':  ('#2b0d0d', '#4d1a1a', '#ef4444', '#f8a0a0'),
    }
    ...
    st.markdown(
        ...
        f"<p style='color:#7a8aaa;font-size:.88rem'>{ri['icon']} {ri['name']} 的结局</p>"
    ...
    dot_html = ''.join(
        ...
        f"background:{'#34c97a' if s > 0 else '#ef4444' if s < 0 else '#f59e0b'};"
        f"margin:0 5px;box-shadow:0 0 8px {'#34c97a66' if s > 0 else '#ef444466' if s < 0 else '#f59e0b66'}'>"
        f"</span>"
    ...
    st.markdown(
        ...
        f"<p style='color:#c8d0e8'>{VERDICT_TEXT[cat]}</p></div>",
        unsafe_allow_html=True
    )

    tag_map = {
        'good': [('✅ 合规导向','#0d2b1e','#34c97a'), ('✅ 职业审慎','#0a1e3e','#4f8ef7'), ('✅ 风险识别','#0a1e3e','#4f8ef7')],
        'mid':  [('⚠️ 压力妥协','#2b1f0a','#f59e0b'), ('⚠️ 信息不对称','#0a1e3e','#4f8ef7'), ('⚠️ 灰色地带','#2b1f0a','#f59e0b')],
        'bad':  [('❌ 激进处理','#2b0d0d','#ef4444'), ('❌ 利润驱动','#2b0d0d','#ef4444'), ('⚠️ 监管风险','#2b1f0a','#f59e0b')],
    }
    tags_html = ''.join(
        f"<span style='display:inline-block;background:{bg};color:{fg};border:1px solid {fg}44;"
